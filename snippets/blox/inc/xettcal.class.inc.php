...
class xettcal
{
    ...
    function getMonthCal($year, $month)
    {
        ...
        while ($afterlastday == 0) {
            for ($daycount = 1; $daycount <= 7; $daycount++) {
                if (($daycount == $firstweekdaynum) && ($afterlastday == 0)) $dayisempty = 0;
            ...
            if ($daytimestamp == 'dayisempty') {
                ...
                if ($this->events_grouped == '1') {
                    ...
                    foreach ($events as $key => $groupevents) {
                        ...
                        if (is_array($tmpevents[0])) $group = array_merge($group, $tmpevents[0]);
                        $dayevents[] = $group;
                    }
                    //print_r($dayevents);
                } else {
    ...
    /////////////////////////////////////////////////
    function extractevents($events, $start, $end, $limit = 0, $sort = false)
    {
        ...
        if (count($events) > 0) {
            foreach ($events as $event) {
                ...
                if ((($eventstart <= $start) && ($eventend >= $start) || ($eventstart >= $start) && ($eventstart <= $end) || ($eventend >= $start) && ($eventend <= $end))) {
                    $extractevents[$event['Time']] = $event;
                }
                
                //echo '<pre>' . print_r($extractevents, true) . '</pre>';
                
                if (count($extractevents) > 0) {
                    if ($sort) {
                        ksort($extractevents);
                    }
                    if (!empty($limit)) {
                        $tempevents = array();
                        foreach ($extractevents as $event) {
                            $tempevents[] = $event;
                        }
                        $tempitems = array();
                        for ($i = 0; $i < $limit; $i++) {
                            if (isset($tempevents[$i])) {
                                $tempitems[] = $tempevents[$i];
                            }
                        }
                        $extractevents = $tempitems;
                    }
    ...
    function getEvents($startDate = '0000-00-00', $endDate = '0000-00-00', $contentFields = null, $limit = 0, $orderDir = 'ASC', $nodes = 0, $getAll = '0', $getTVs = '')
    {
        ...
        // Verify parameters
        if (('0000-00-00' != $startDate && '0000-00-00' != $endDate && $startDate == date('Y-m-d', strtotime($startDate)) || $endDate == date('Y-m-d', strtotime($endDate))) || ($getAll !== '0')) {
