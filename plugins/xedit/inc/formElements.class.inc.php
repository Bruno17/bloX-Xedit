...
class formElements{
	...
	function renderFormElement($field_type, $field_name, $default_text, $field_elements, $field_value, $field_style='', $attributes='',$classnames=array()) {
		global $base_url;
		global $rb_base_url;
		...
		switch ($field_type) {
			...
			case "number": // handles the input of numbers
				$field_html .=  '<input '.$class.$attributes.' type="text" id="tv'.$field_name.'" name="tv'.$field_name.'" value="'.htmlspecialchars($field_value).'" '.$field_style.' tvtype="'.$field_type.'" onchange="documentDirty=true;" style="width:100%" />';
				break;
			case "textareamini": // handler for textarea mini boxes
				$field_html .=  '<textarea '.$class.$attributes.' id="tv'.$field_name.'" name="tv'.$field_name.'" cols="40" rows="5" onchange="documentDirty=true;" style="width:100%">' . htmlspecialchars($field_value) .'</textarea>';
				break;
			...
			case "richtext": // handler for textarea boxes
				$field_html .=  '<textarea '.$class.$attributes.' id="tv'.$field_name.'" name="tv'.$field_name.'" cols="40" rows="15" onchange="documentDirty=true;" style="width:100%;">' . htmlspecialchars($field_value) .'</textarea>';
				break;
			case "date":
                if($field_value=='') $field_value=0;
                $cal = 'cal' . str_replace('-','_',$field_name);

				$field_html .=  '<input '.$class.$attributes.' id="tv'.$field_name.'" name="tv'.$field_name.'" type="hidden" value="' . ($field_value==0 || !isset($field_value) ? "" : $field_value) . '" onblur="documentDirty=true;">';
			...
			case "dropdown": // handler for select boxes
				$field_html .=  '<select '.$class.$attributes.' id="tv'.$field_name.'" name="tv'.$field_name.'" size="1" onchange="documentDirty=true;">';
			...
			case "listbox": // handler for select boxes
				$field_html .=  '<select '.$class.$attributes.' id="tv'.$field_name.'" name="tv'.$field_name.'" onchange="documentDirty=true;" size="8">';	
				$index_list = $this->ParseIntputOptions(ProcessTVCommand($field_elements, $field_name));
				while (list($item, $itemvalue) = each ($index_list))
				{
					list($item,$itemvalue) =  (is_array($itemvalue)) ? $itemvalue : explode("==",$itemvalue);
			...
			case "listbox-multiple": // handler for select boxes where you can choose multiple items
				...
				$field_html .=  '<select  '.$class.$attributes.' id="tv'.$field_name.'[]" name="tv'.$field_name.'[]" multiple="multiple" onchange="documentDirty=true;" size="8">';
				$index_list = $this->ParseIntputOptions(ProcessTVCommand($field_elements, $field_name));
				while (list($item, $itemvalue) = each ($index_list))
				{
					list($item,$itemvalue) =  (is_array($itemvalue)) ? $itemvalue : explode("==",$itemvalue);
			...
			case "url": // handles url input fields
				...
				$field_html ='<table border="0" cellspacing="0" cellpadding="0"><tr><td><select '.$class.$attributes.' id="tv'.$field_name.'_prefix" name="tv'.$field_name.'_prefix" onchange="documentDirty=true;">';
				foreach($urls as $k => $v){
				...
				$field_html .=  '<input '.$class.$attributes.' type="text" id="tv'.$field_name.'" name="tv'.$field_name.'" value="'.htmlspecialchars($field_value).'" width="100" '.$field_style.' onchange="documentDirty=true;" /></td></tr></table>';
				break;
			case "checkbox": // handles check boxes
				...
				while (list($item, $itemvalue) = each ($index_list))
				{
					list($item,$itemvalue) =  (is_array($itemvalue)) ? $itemvalue : explode("==",$itemvalue);
					if (strlen($itemvalue)==0) $itemvalue = $item;
					$field_html .=  '<input '.$class.$attributes.' type="checkbox" value="'.htmlspecialchars($itemvalue).'" id="tv_'.$i.'" name="tv'.$field_name.'[]" '. (in_array($itemvalue,$field_value)?" checked='checked'":"").' onchange="documentDirty=true;" /><span class="label">'.$item.'</span>';
					$i++;
				}
				break;
			case "option": // handles radio buttons
				...
				while (list($item, $itemvalue) = each ($index_list))
				{
					list($item,$itemvalue) =  (is_array($itemvalue)) ? $itemvalue : explode("==",$itemvalue);
					if (strlen($itemvalue)==0) $itemvalue = $item;
					$field_html .=  '<input '.$class.$attributes.' type="radio" value="'.htmlspecialchars($itemvalue).'" name="tv'.$field_name.'" '.($itemvalue==$field_value ?'checked="checked"':'').' onchange="documentDirty=true;" /><span class="label">'.$item.'</span>';
				}
				break;
			...
			default: // the default handler -- for errors, mostly
				$field_html .=  '<input '.$class.$attributes.' type="text" id="tv'.$field_name.'" name="tv'.$field_name.'" value="'.htmlspecialchars($field_value).'" '.$field_style.' onchange="documentDirty=true;" />';
...
return $this->renderFormElement($tv['type'], $tv['formname'], $defaultOutput, $tv['elements'], ($tvPBV?$tvPBV:$tv['value']), ' style="width:300px;"', $attributes, $classnames);
}	
